...
public:
    MyCall(Account &acc, int call_id = PJSUA_INVALID_ID): Call(acc, call_id), ex_media(NULL) {
        myAcc = (MyAccount *)&acc;
    }
    void setMedia(AudioMedia *_ex_media) {
        ex_media = _ex_media;
    }
...
void MyCall::onCallMediaState(OnCallMediaStateParam &prm) {
    ...
    for(uint16_t i = 0;i < ci.media.size();i ++) {
        if(ci.media[i].type == PJMEDIA_TYPE_AUDIO && getMedia(i)) {
            ...
            if(ex_media) {
                aud_med->startTransmit(*ex_media);
                ex_media->startTransmit(*aud_med);
            } else {
...
static void mainProg1(Endpoint &ep) throw(Error) // Make Call
{
    ...
    acc_cfg.idUri = "sip:101@10.128.171.151";
    acc_cfg.regConfig.registrarUri = "sip:10.128.171.151";
    acc_cfg.sipConfig.authCreds.push_back( AuthCredInfo("10.128.171.151", "*",
                                                        "101", PJSIP_CRED_DATA_PLAIN_PASSWD, "101") ); // pjsip/include/pjsua2/siptypes.hpp
    MyAccount *acc(new MyAccount);
    acc->create(acc_cfg);
    std::cout<<"*** after creating ***"<<std::endl;
    ep.audDevManager().setNullDev();
    /* And install sound device using Extra Audio Device */
    int fd_in = open("send_input.txt", O_RDWR);
    int fd_out = open("send_output.txt", O_RDWR|O_APPEND);
    ExtraAudioDevice auddev2(fd_in, fd_out);
    try {
	    auddev2.open();
    } catch (...) {
    ...
    call->setMedia(&auddev2);
    acc->calls.push_back((Call *)call);
    CallOpParam prm(true);
    prm.opt.audioCount = 1;
    prm.opt.videoCount = 0;
    std::cout<<"*** before making call ***" << std::endl;
    call->makeCall("sip:102@10.128.171.151", prm);
    std::cout<<"*** after making call ***"<<std::endl;
